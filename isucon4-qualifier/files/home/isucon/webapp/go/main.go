...
func init() {
	...
	if err != nil {
        ...
        prepareInsertLog, err = db.Prepare("INSERT INTO login_log (`created_at`, `user_id`, `login`, `ip`, `succeeded`) VALUES (?,?,?,?,?)")
	if err != nil {
		panic(err)
	}
...
func main() {
	m := martini.Classic()
        m.Map(log.New(ioutil.Discard, "[martini]", log.LstdFlags))
